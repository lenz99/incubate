...
\begin{split}
π = P(C<X) &= ∫_α^∞ f_X(x) · F_C(x) dx\\
  &= ∫_α^∞ \frac{k}{σ} · (\frac{x-α}{σ})^{k-1} \exp(-(\frac{x-α}{σ})^k) \frac{x-α}{Z-α} dx\\
  &= ∫_0^∞ \frac{k}{σ} · (\frac{t}{σ})^{k-1} \exp(-(\frac{t}{σ})^k) \frac{t}{Z-α} dt\\
  &= \frac{k}{σ(Z-α)} · ∫_0^∞ t · \frac{k}{σ} · (\frac{t}{σ})^{k-1} \exp(-(\frac{t}{σ})^k) dt\\
  &= \frac{k}{σ(Z-α)} · \Big\{ [-t · \frac{σ}{k} · \exp(-(\frac{t}{σ})^k)]_{t=0}^{t=\infty} - ∫_0^∞ -\frac{σ}{k} · \exp(-(\frac{t}{σ})^k) dt  \Big\}\\
  &= \frac{k}{σ(Z-α)} · \Big\{ [0 - 0] + \frac{σ}{k} ∫_0^∞ \exp(-(\frac{t}{σ})^k) dt \Big\}\\
  &= \frac{1}{Z-α} · ∫_0^∞ \exp(-(\frac{t}{σ})^k) dt\\
  &= \frac{σ}{Z-α} · ∫_0^∞ \exp(-u^k) du\\
  &= \frac{σ}{Z-α} · \frac{1}{k} · Γ(\frac{1}{k})
\end{split}
\end{equation*}
\]
...
We can solve for $Z$:
\[
π · (Z-α) = σ · \frac{1}{k} · Γ(\frac{1}{k}) \qquad ⇔ \qquad
Z = α + \frac{σ}{π k} · Γ(\frac{1}{k})
%%% Z = \frac{α + \frac{σ}{k}·Γ(\frac{1}{k})}{π}
\]
