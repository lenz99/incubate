...
where γ(,) is the incomplete gamma function (not regularized).
$$
\begin{equation*}
\begin{split}
...
\end{equation*}
$$
We can solve for $r$ by means of root finding. We consider the function on log-scale, namely
$$
f_k(r) = \log(k) - k \log(r) + \log(\gamma(k, r)) - \log(π)
$$
For given $k$, it is a monotonically decreasing function in $r$. A root $r_0$ of $f_k$ gives us the solution for $Z$,
namely $Z = α + σ r_0^{\frac{1}{γ}}$.
