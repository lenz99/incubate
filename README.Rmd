...
Concretely, employing MPSE, `incubate` will
...
The `incubate`-package provides the delayed exponential distribution as a special case of the delayed Weibull distribution.
...
To switch on parallel computation, e.g. for bootstrap parameter tests or power simulations, simply set up a suitable computation plan via the `future`-framework.
...
That's it. You do *not* have to change any function calls as `incubate` is `future`-aware.
Consult the documentation of the [futureverse website](https://www.futureverse.org/) for more information about this framework and its supported computation plans.
...
Use R's standard `install.packages` to install `incubate` from CRAN as usual, i.e.
