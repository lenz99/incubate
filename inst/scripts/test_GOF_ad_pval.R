...
pfun_exp <- stats::approxfun(x = c(0, qq_exp, +Inf),
...
A2_crit_weib <- tibble::tribble(
  ~shape_inv, ~`a=0.5`, ~`a=0.25`, ~`a=0.15`, ~`a=0.1`, ~`a=0.05`, ~`a=0.025`, ~`a=0.01`, ~`a=0.005`,
...
pfun_weib_fixedShape <- purrr::map(.x = unique(A2_crit_weib$shape_inv),
                      .f = ~ {
                        ...
                        ml <- rmetalog::metalog(x = dfsub$statist, bounds = 0, boundedness = 'sl',
                                                ...
                                                probs = dfsub$prob)
                        stats::approxfun(x = c(0, qq_weib, +Inf),
                                         y = c(0, 1 - rmetalog::pmetalog(ml, q = qq_weib, term = 4), 1), method = 'linear')
                      })
...
pfun_weib <- function(q, shape) {
  ...
  purrr::map(.x = unique(q),
             .f = function(qu) af=stats::approxfun(x=as.numeric(names(pfun_weib_fixedShape)),
