...
if (any(c('help', 'h') %in% names(cmdArgs))) {
  ...
  cat('  --type=\t what type of results to gather? "test" (default) or "confint" or "estim"\n')
...
myType <- match.arg(arg = tolower(myType), choices = c("test", "confint", "estim"))
...
indOffset <- 0L
RES_FILEN <- c(res  = file.path(myResultsDir, paste0('simRes_', myType, '_', myResultsTag,'.rds')),
               data = file.path(myResultsDir, paste0('simRes_', myType, '_', myResultsTag,'_data.rds'))
)
...
simResDataFileNames <- list.files(myResultsDir,
...
cat(length(simResFileNames), " temporary result files found in sub-directory ",
    sQuote(myResultsDir),"!\n", sep = "")
cat("\n  * ")
...
if (file.exists(RES_FILEN[["res"]])) {
	...
	if (!is.list(resPrev) || !is_named(resPrev)) {
	  ...
	  if (myType == "test") {
	    indOffset <- max(0L,
	                     which(RUN_NS %in% purrr::map_chr(resPrev,
	                                                      .f = ~ {
	                                                        substr(.x[['run']][[1L]],
	                                                               start = 1L, stop = nchar(RUN_NS[[1L]]))
	                                                      })),
	                     na.rm = TRUE)
	    cat(glue("Offset index from saved results is {indOffset}."), "\n")
	  } else {
	    cat("This type ", myType, "is not (yet) properly supported for finding index offset from previous results!\n")
	    q(save = "no")
	  }
	}#fi resPrev
...
resDataCandidates <- purrr::imap(.x = simResDataFileNames, .f = readResultFile,
                                 isDataFile = TRUE) |>
  # drop imap's additional list level
  purrr::list_flatten()
...
if (is.null(resPrev)) {
  ...
  if (haveResDataFiles) {
    cat("Sorry, merging results with previous results is not supported when we have previous DATA result files as well!\n")
    q(save = "no")
  }
...
if (myRemoveTemp) {
  ...
  if (haveResDataFiles) {
    ...
    if (sum(fnRmv)) {
      cat("Successfully removed ", sum(fnRmv), " temporary result DATA files.\n")
      cat("\n  * ")
